// seengroup-backend/src/routes/contact.js
...
/**
 * POST /api/contact
 * İletişim formu (ürün gereksinimleri dahil)
 */
router.post(
  '/',
  contactRateLimiter,
  [
    ...
    body('requirements').optional(),       // string | array | object
    body('productContext').optional(),     // string | object
  ...
  asyncHandler(async (req, res) => {
    ...
    // --- Turnstile doğrulaması (iyileştirilmiş) ---
    ...
    if (captchaEnabled && (process.env.SKIP_CAPTCHA || 'false').toLowerCase() !== 'true') {
      ...
      try {
        if (turnstileSecret) {
          // Cloudflare/Proxy arkasında IP'yi iletelim
          const remoteIp =
          ...
          // (İsteğe bağlı) hostname beyaz liste:
          // if (verifyJson.hostname && !/^(www\.)?seengrp\.com$/i.test(verifyJson.hostname)) {
          //   return res.status(400).json({ success: false, error: 'Captcha hostname mismatch' });
          // }
        }
      } catch (captchaErr) {
    ...
    try {
      // requirements & productContext parse
      ...
      if (productContext) {
        try {
          ...
          logger.warn('Invalid product context format:', e);
        }
      }
      ...
      const newSubmission = await prisma.contact_submissions.create({
        data: {
          ...
          message: safeMessage,
          requirements: Array.isArray(parsedRequirements) && parsedRequirements.length > 0 ? JSON.stringify(parsedRequirements) : null,
      ...
      // kullanıcıya otomatik mail (non-blocking)
      try {
...
${safeMessage ? `Message: ${safeMessage}\n` : ''}${
          ...
          sendEmail(process.env.ADMIN_EMAIL, adminSubject, adminHtml, adminText, { replyTo: safeEmail }).catch((err) =>
            logger.warn('Admin contact notification email error:', err)
          );
        }
      } catch (adminMailErr) {
...
router.get('/submissions', protect, authorize('ADMIN'), asyncHandler(async (req, res) => {
  try {
    ...
    const submissions = submissionsRaw.map((s) => {
      ...
      try { if (typeof requirements === 'string') requirements = JSON.parse(requirements); } catch {}
      try { if (typeof productContext === 'string') productContext = JSON.parse(productContext); } catch {}
...
router.get('/stats', protect, authorize('ADMIN'), asyncHandler(async (req, res) => {
  try {
    ...
    const [totalSubmissions, recentSubmissions, countryStats, statusStats] = await Promise.all([
      ...
      prisma.contact_submissions.count({ where: { createdAt: { gte: weekAgo } } }),
    ...
    countryStats.forEach((stat) => { countryStatsObj[stat.country] = stat._count.country; });

    const statusStatsObj = {};
    statusStats.forEach((stat) => { statusStatsObj[stat.status] = stat._count.status; });
    ...
    res.status(500).json({ success: false, error: 'Failed to get contact statistics' });
  }
}));
...
router.get('/submissions/:id', protect, authorize('ADMIN'), asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to get contact submission' });
  }
}));

/**
 * PATCH /api/contact/submissions/:id  (ADMIN)
 */
router.patch(
  '/submissions/:id',
  protect,
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to update contact submission' });
    }
  })
);

/**
 * DELETE /api/contact/submissions/bulk  (ADMIN)
 */
router.delete('/submissions/bulk', protect, authorize('ADMIN'), asyncHandler(async (req, res) => {
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ success: false, error: 'Submission IDs array is required' });
    }

    const validIds = ids.filter((id) => typeof id === 'string' && id.trim() !== '');
    if (validIds.length === 0) {
      return res.status(400).json({ success: false, error: 'No valid submission IDs provided' });
    }
...
router.delete('/submissions/:id', protect, authorize('ADMIN'), asyncHandler(async (req, res) => {
  try {
    ...
    const submission = await prisma.contact_submissions.findUnique({ where: { id } });
    if (!submission) {
      return res.status(404).json({ success: false, error: 'Contact submission not found' });
    }

    await prisma.contact_submissions.delete({ where: { id } });
