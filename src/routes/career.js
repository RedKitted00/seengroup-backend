...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `New job application received for: ${job.title} from: ${email}`
      );
      ...
      try {
        ...
        sendAdminNotification(emailData)
          .then((result) => {
            if (result.success) {
              logger.info(
                `Admin notification sent for application ${application.id}`
              );
            } else {
              logger.error(
          ...
          .catch((error) => {
            logger.error(
              `Error sending admin notification for application ${application.id}:`,
              error
            );
          });
      } catch (emailError) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const jobTitle =
        position && position.trim() ? position.trim() : "General Application";
      ...
      if (!generalJob) {
        generalJob = await prisma.jobs.create({
          data: {
            title: jobTitle,
            description:
              "General job application for positions not currently listed",
            type: "FULL_TIME",
            isActive: true,
            updatedAt: new Date(),
      ...
      try {
        ...
        sendAdminNotification(emailData)
          .then((result) => {
            if (result.success) {
              logger.info(
                `Admin notification sent for general application ${application.id}`
              );
            } else {
              logger.error(
                `Failed to send admin notification for general application ${application.id}:`,
                result.error
              );
            }
          })
          .catch((error) => {
            logger.error(
              `Error sending admin notification for general application ${application.id}:`,
              error
            );
          });
      } catch (emailError) {
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        resumeUrl = uploadResult.url;
        if (!resumeUrl) {
          logger.error("Upload returned no URL for resume", {
            originalname,
            mimetype,
          });
          return res
            .status(500)
            .json({ success: false, error: "Failed to upload resume file" });
        }
      } catch (uploadErr) {
        logger.error("Resume upload error:", uploadErr);
        return res
          .status(500)
          .json({ success: false, error: "Failed to upload resume file" });
      }
      ...
      if (coverFile) {
        try {
          ...
          return res
            ...
            .json({
              success: false,
              error: "Failed to upload cover letter file",
            });
        }
      }
      ...
      logger.info(
        `New job application received from: ${email} for position: ${
          job?.title || position
        }`
      );
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        return res.status(404).json({
          success: false,
          error:
            "Resume file not found on server. This may be an old upload that was not migrated to cloud storage.",
        });
      }
      ...
      logger.info(
        `Serving legacy file: ${fileName}, size: ${stats.size} bytes`
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${application.name}_resume_${fileName}"`
      );
    ...
    try {
      ...
      if (!coverUrl || !coverUrl.startsWith("http")) {
        return res
          ...
          .json({
            success: false,
            error: "No cover letter file found for this application",
          });
      }
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to download cover letter" });
    }
  })
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      !action ||
      !["approve", "reject", "review", "delete"].includes(action)
    ) {
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      switch (action) {
        ...
        case "delete":
          ...
          logger.info(
            `Bulk deleted ${applicationIds.length} applications by admin: ${req.user.email}`
          );
      ...
      logger.info(
        `Bulk ${action} ${result.count} applications by admin: ${req.user.email}`
      );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!subject || !message) {
      return res
        .status(400)
        .json({ success: false, error: "Subject and message are required" });
    }
    ...
    try {
      ...
      if (!application || !application.email) {
        return res
          .status(404)
          .json({ success: false, error: "Application or email not found" });
      }
    ...
    `;
      ...
      const result = await sendEmail(application.email, subject, html, text, {
        replyTo,
        cc,
        bcc,
      });
      ...
      logger.info(
        `Applicant email sent for application ${id} to ${application.email}`
      );
      res
        .status(200)
...
router.patch(
  ...
  [
    body("status")
      .isIn(["PENDING", "REVIEWING", "APPROVED", "REJECTED", "HIRED"])
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Application status updated: ${id} to ${status} by admin: ${req.user.email}`
      );
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { id } = req.params;
    const { title, description, requirements, location, type, isActive } =
      req.body;
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Application status updated to ${status} for: ${application.email}`
      );
